// GBX LP Announce — publishes GBX:LP:<https-url> on-chain via OP_RETURN.
// Anti-spam: L1 fee + liveness window (re-announce every ~200k blocks or expire).
// Usage: GBX_CLI=goldbrix-cli GBX_DATADIR=... [GBX_WALLET=...] node announce-lp.js <https-url>
...
(async () => {
  ...
  if (!url || !/^https:\/\/[a-z0-9.-]+(:\d+)?(\/[a-zA-Z0-9._\/-]*)?$/.test(url)) {
    console.error('usage: node announce-lp.js <https-url>  (https only)'); process.exit(1);
  }
  const msg = `GBX:LP:${url}`;

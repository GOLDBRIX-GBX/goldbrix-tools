...
// Branding: GBX only. Deps are injected (the browser wires them to crypto-utils/evm-tx/relay/daemon).
...
// BUY GBX with USDC (atomic, non-custodial). ctx.evm/gbx/submitIntent injected.
export async function buyGbx(ctx){
  ...
  if (ctx.gasless) {
    // GASLESS: the user signs the EIP-3009 authorization (zero gas), the relayer (daemon) submits lockAuth.
    const g = ctx.gasless;
  ...
  if(!swap) throw new Error('timeout: LP did not lock GBX -> refund USDC after T1');
  ...
  if(!v.ok) throw new Error('GBX HTLC invalid ('+v.reason+') -> not claiming, refund USDC after T1');
...
// ===== SELL (GBX -> USDC), symmetric with buyGbx. USER locks GBX, LP locks USDC, USER claims USDC (reveals secret). =====
...
export async function sellGbx(ctx){
  ...
  if(!usdcLock) throw new Error('timeout: LP did not lock USDC -> refund GBX after T1');
  const v = verifyUsdcLock({ ev: usdcLock, userEvmAddr, Hhex, usdcAddr, minAmount: usdcAmount });
  if(!v.ok) throw new Error('USDC lock invalid ('+v.reason+') -> not claiming, refund GBX after T1');

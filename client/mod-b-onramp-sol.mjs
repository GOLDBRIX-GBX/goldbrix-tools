...
const JUP_BASE  = "https://lite-api.jup.ag/swap/v1";                 // endpoint gratuit, fara API key
const RPCS = [
...
export async function quoteSolToUsdc(solAmountLamports, slippageBps){
  ...
  return {
    ...
    outUsdc6:   Number(q.outAmount),                 // USDC cu 6 zecimale
...
export async function swapSolToUsdc(ctx){
  ...
  // 4. submit on-chain prin RPC public (fondatorul NU e in flux)
...
// SWAP Raydium: quote proaspat -> build tx LEGACY -> user semneaza -> submit (RPC fallback)
export async function swapRaydium(ctx){
  ...
  // Raydium cere ATA-urile explicit (REQ_INPUT_ACCOUT_ERROR fara ele)

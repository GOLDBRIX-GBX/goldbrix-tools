...
(function(){
  ...
  function chainLabel(ch){ return ch==='solana'?'Solana':ch==='arbitrum'?'Arbitrum':ch==='bsc'?'BNB':'Base'; }
  function cfg(type){
  ...
  function lines(a){
    ...
    else if(a.type==='GBX_BUY'){ main='USDC → GBX'; sub='+'+fmtGbx(a.gbx_amount)+' GBX · −'+(Number(a.usdc)||0).toFixed(2)+' USDC · '+chainLabel(a.chain); }
    else if(a.type==='GBX_SELL'){ main='GBX → USDC'; var _g=Number(a.gbx_amount)||0; sub=(_g>0?('−'+fmtGbx(_g)+' GBX · '):'')+'+'+(Number(a.usdc)||0).toFixed(2)+' USDC · '+chainLabel(a.chain); }
    else if(a.type==='CONVERT'){ main=(a.cv_from||'')+' → '+(a.cv_to||''); sub='−'+fmtNum(a.cv_in)+' '+(a.cv_from||'')+' · +'+fmtNum(a.cv_out)+' '+(a.cv_to||'')+' · '+chainLabel(a.chain)+(a.via?(' · '+a.via):''); }
      ...
      try{
        ...
        try{
          ...
          (Array.isArray(sw)?sw:[]).forEach(function(s){
            if(s.type==='CONVERT'){ acts.push({type:'CONVERT',chain:s.chain,cv_from:s.cv_from,cv_to:s.cv_to,cv_in:Number(s.cv_in)||0,cv_out:Number(s.cv_out)||0,via:s.via,tx_hash:s.tx_hash||'',timestamp:s.timestamp,status:s.status||'completed'}); }
            else if(s.type==='GBX_BUY' || s.type==='GBX_SELL'){
              acts.push({type:s.type, gbx_amount:Number(s.gbx_amount)||0, usdc:Number(s.usdc)||0,
                timestamp:s.timestamp||Date.now(), tx_hash:s.tx_hash||'', status:s.status||'completed', chain:(s.chain||'base')});
            }
          });
